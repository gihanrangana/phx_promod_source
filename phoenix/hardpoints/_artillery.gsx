...
startBarrage( endPos, startPos )
{
	...
	self thread phoenix\_common::notifyTeamLn( "Friendly artillery called by^1 " + self.name );
	...
	while( isDefined( ent ) )
	{
		...
		if( getTime() - ent.fireTime > 500 )
		{
			...
			if( collision[ "surfacetype" ] != "default" && collision[ "fraction" ] < 1 ) 
			{
				...
				thread phoenix\_common::playSoundinSpace( "artillery_impact", collision[ "position" ] );
