...
init()
{
	...
	precacheShader("jen_score");
	precacheShader("jen_black");

	game["strings"]["draw"] = "DRAW";
	game["strings"]["round_draw"] = "ROUND DRAW";
	game["strings"]["round_win"] = "ROUND WIN";
	game["strings"]["round_loss"] = "ROUND LOSS";
	game["strings"]["victory"] = "VICTORY";
	game["strings"]["defeat"] = "DEFEAT";
	game["strings"]["halftime"] = "HALFTIME";
	game["strings"]["overtime"] = "OVERTIME";
	game["strings"]["roundend"] = "ROUND END";
	game["strings"]["intermission"] = "INTERMISSION";
	game["strings"]["side_switch"] = "SWITCH SIDE";
	game["strings"]["match_bonus"] = "MATCH BONUS IS";

	level thread onPlayerConnect();

	level.windowheight2 = 150;
	level.windowwidth2 = 150;
	level.borderwidth2 = 300;

	level.windowheight = 100;
	level.windowwidth = 160;
	level.borderwidth = 30;
}
...
teamOutcomeNotify(winner, isRound, endReasonText, delay)
{
	...
	outcomeTitle setPoint("TOP", undefined, 0, 55);
	...
	outcomeText setPoint("TOP", "BOTTOM", 0, 15);
	outcomeText.glowAlpha = 0;
	outcomeText.hideWhenInMenu = false;
	outcomeText.archived = false;
	outcomeText.fontScale = 1.4;

	//**********************************************main middle  shader
	iconshade = createFontString(font, titleSize);
	//	iconshade setPoint( "TOP", undefined, 0, 115 );  //  -150 means left , 140 means down
	iconshade.horzAlign = "fullscreen";
	iconshade.vertAlign = "fullscreen";
	iconshade.hideWhenInMenu = false;
	iconshade.archived = false;
	iconshade.sort = 0;
	/*****************************************cover outcomecometitle with black shader****/
	iconshade2 = createFontString(font, titleSize);
	iconshade2 setPoint("TOP", undefined, 0, 55);
	iconshade2.glowAlpha = 1;
	iconshade2.hideWhenInMenu = false;
	iconshade2.archived = false;
	/************************under victory Shader**********/
	iconshade3 = createFontString(font, 2);
	iconshade3 setParent(outcomeTitle);
	iconshade3 setPoint("TOP", "BOTTOM", 0, 10);
	iconshade3.glowAlpha = 0;
	iconshade3.hideWhenInMenu = false;
	iconshade3.archived = false;

	outcomeTitle.sort = 9999;
	outcomeText.sort = 9999;
	iconshade.sort = 0;
	iconshade2.sort = 0;
	iconshade3.sort = 0;
	...
	if (winner == "halftime")
	{
		...
		iconshade SetShader("jen_score", 640, 480);
		iconshade2 SetShader("jen_black", level.windowwidth + level.borderwidth + level.borderwidth, 40);
		iconshade3 SetShader("jen_black", 180, 25);
	}
	...
	rightIcon setPoint("TOP", "BOTTOM", 160, 65);
	rightIcon.hideWhenInMenu = false;
	rightIcon.archived = false;
	rightIcon.glowAlpha = 1;
	...
	if (winner == "halftime")
	{
		...
		leftScore setPoint("TOP", "BOTTOM", -220, -50);
		rightScore MoveOverTime(2.5);
		rightScore setPoint("TOP", "BOTTOM", 220, -50);
	}
	...
	self thread resetTeamOutcomeNotify(outcomeTitle, outcomeText, leftIcon, rightIcon, leftScore, rightScore, matchBonus, iconshade, iconshade2, iconshade3);
}
...
outcomeNotify(winner, endReasonText, delay)
{
	...
	iconshade = createFontString(font, titleSize);
	iconshade setPoint("TOP", undefined, 0, spacing);

	iconshade2 = createFontString(font, titleSize);
	iconshade2 setPoint("TOP", undefined, 0, 55);
	iconshade3 = createFontString(font, 2);
	iconshade3 setPoint("TOP", undefined, 0, 47);
	iconshade3 setParent(outcomeTitle);
	...
	outcomeText = createFontString(font, 1.4);
	outcomeText setParent(outcomeTitle);
	outcomeText setPoint("TOP", "BOTTOM", 0, 15);
	...
	self thread resetOutcomeNotify(outcomeTitle, outcomeText, firstTitle, secondTitle, thirdTitle, matchBonus, iconshade, iconshade2, iconshade3);
}

resetOutcomeNotify(outcomeTitle, outcomeText, firstTitle, secondTitle, thirdTitle, matchBonus, iconshade, iconshade2, iconshade3)
{
	self endon("disconnect");
	...
	if (isDefined(iconshade))
		iconshade destroyElem();
	if (isDefined(iconshade2))
		iconshade2 destroyElem();
	if (isDefined(iconshade3))
		iconshade3 destroyElem();
}

resetTeamOutcomeNotify(outcomeTitle, outcomeText, leftIcon, rightIcon, LeftScore, rightScore, matchBonus, iconshade, iconshade2, iconshade3)
{
	self endon("disconnect");
	self waittill("reset_outcome");
